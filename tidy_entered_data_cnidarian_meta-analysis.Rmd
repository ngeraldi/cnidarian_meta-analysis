...
## newer corrections march 2020
corr2_1<-openxlsx::read.xlsx(paste(dat_file2,"Column fix_File 1_mar2020.xlsx", sep="" ), startRow=1,sheet = 1, check.names=T)
corr2_2<-openxlsx::read.xlsx(paste(dat_file2,"Column fix_File 2_mar2020.xlsx", sep="" ), startRow=1,sheet = 1, check.names=T)
corr2_1<-mutate_all(corr2_1, funs(tolower)) # make all column names lower case  -- always do this for any meta-analysis**
corr2_2<-mutate_all(corr2_2, funs(tolower))
corr_temp<-openxlsx::read.xlsx(paste(dat_file2,"Temperature fix_file.xlsx", sep="" ), startRow=1,sheet = 1, check.names=T) # names(corr_temp)
corr_temp<-corr_temp[,c(1:8,11,12,13)]
...
land<-read.table(paste(dat_file, "global_human_and_abiotic_present.csv",sep="" ), sep=",",header=T) # names(land)
vel<-read.table(paste(dat_file, "climate_velocities.csv",sep="" ), sep=",",header=T) # names(vel)
...
variable.label.fix.2.fix_PAM is used for analysis down line !!!!!!!
changed to Trait.type in tidy_dat5 !!!!!!!!!!
...
## fixes 2
fixes March 2020 
uses corr2_1 and corr2_2 imported sheets
!!!!  variable.label.fix.2.fix_PAM will become Trait.type !!!!!
```{r fix2}
names(dat1)
unique(dat1$variable.unit)
unique(dat1$variable.type)
unique(dat1$variable.label)

## fix corr2_1 for names
corr2_1 <- corr2_1 %>% # names(corr2_1)
    dplyr::rename(Retriever=Extractor , Original.ref.number = Reference.ID , variable.label.fix.2.fix_PAM = Trait.type) %>% 
    dplyr::select(-Trait.units) %>% 
    mutate(Original.ref.number = as.numeric(Original.ref.number))
corr2_2 <- corr2_2 %>% # names(corr2_2)
    dplyr::rename(Original.ref.number = Reference.ID , variable.label.fix.2.fix_PAM = Trait.type, variable.unit=Trait.units) %>% 
    mutate(Original.ref.number = as.numeric(Original.ref.number)) %>% 
    dplyr::select(-Stressors.studied_study)
corr_temp2 <- corr_temp %>% # names(corr_temp2)
      dplyr::rename(Original.ref.number = Reference.ID , variable.label.fix.2.fix_PAM = Trait.type, variable.unit=Trait.units,
                    effect.direction = Effect.direction, experimental_temp.level..degrees.=Experimental_temp.level.degrees, 
                    experimental_sample.size=Experimental_sample.size,  experimental_mean.response= Experimental_mean.response_original) %>% 
    mutate(Original.ref.number = as.numeric(Original.ref.number)) %>% 
    dplyr::select(-Stressors.studied_study, -variable.label.fix.2.fix_PAM)
  



#  if CaCO3 appears in "Traits.units" , change growth to calcification under trait.label  
dat1.5 <- dat1 %>%    # names(dat1)
    mutate_if(is.factor, as.character) %>% 
  mutate_at(vars(Retriever,Stressor.studied , Study.type, variable.unit, variable.label.fix.2.fix_PAM), funs(tolower))  %>%
  #  change variable.label to calcification if was growth and measured caco3 
    mutate(variable.label.fix.2.fix_PAM=if_else(grepl("caco", variable.unit, ignore.case=TRUE) & variable.label.fix.2.fix_PAM == "growth", "calcification" , variable.label.fix.2.fix_PAM )) %>% 
  # change variable.label to pam if Fv/Fm in variable.unit
    mutate(variable.label.fix.2.fix_PAM=if_else(grepl("Fv/Fm", variable.unit, ignore.case=TRUE)   , "PAM" , variable.label.fix.2.fix_PAM )) %>% 
  # corrections based on file 1- corr1_2
    mutate_at(vars(Retriever,Stressor.studied , Study.type, variable.unit), funs(tolower))  %>%
    left_join(corr2_1) %>%
    mutate(variable.label.fix.2.fix_PAM=if_else(complete.cases(Trait.type.fix)   , Trait.type.fix , variable.label.fix.2.fix_PAM )) %>% 
    mutate(variable.unit=if_else(complete.cases(Trait.units.fix)   , Trait.units.fix , variable.unit )) %>%
    dplyr::select (-Trait.type.fix , -Trait.units.fix) %>% 
  # corrections based on file 2- corr2_2
    left_join(corr2_2) %>%
    mutate(variable.unit=if_else(complete.cases(Trait.units.fix)   , Trait.units.fix , variable.unit )) %>%
    dplyr::select ( -Trait.units.fix) %>%  ###   unique(dat1.5$variable.unit)
  # other unit corrections
    mutate(variable.unit=if_else(grepl("prey consumed h-1 cm-2", variable.unit)   , "prey consumed h-1 cm-2 ml-1" , variable.unit )) %>%
    mutate(variable.unit=if_else(grepl("µmol electrons m-2 s", variable.unit)   , "µmol electrons m-2 s-1" , variable.unit )) %>% 
    mutate(variable.unit=if_else(grepl("mol caco3", variable.unit) & Original.ref.number==258 &  variable.label.fix.2.fix_PAM=="respiration" , "umol O2 g-1 h-1" , variable.unit )) %>% 
      mutate(variable.unit=if_else(grepl("net pmax", variable.unit) & Original.ref.number==772, "net pmax (o2 flux) cm2" , variable.unit )) %>% 
        mutate(variable.unit=if_else(grepl("umol caco3 cm", variable.unit) & Original.ref.number==158, "umol caco3 cm-1 d-1" , variable.unit )) %>% 
  
         mutate(variable.unit=if_else(grepl("nmol ca mg spicules", variable.unit) & Original.ref.number==805, "nmol ca mg spicules h-1" , variable.unit )) %>% 
  
    mutate(variable.unit=gsub( "µ" , "u", variable.unit)) %>% #  
  mutate(variable.unit=gsub( "μ" , "u", variable.unit)) %>%
    mutate(variable.unit=gsub( "\\^" , "-", variable.unit)) %>% #  unique(dat1)
  # temp corrections using names(corr_temp2)
    left_join(corr_temp2) %>% 
    mutate(experimental_temp.level..degrees.=if_else(complete.cases(Experimental_temp.level.degrees_fix)   , Experimental_temp.level.degrees_fix , experimental_temp.level..degrees. )) %>%
      dplyr::select(-Experimental_temp.level.degrees_fix) %>% 
 # CO2 corrections
    mutate(experimental_pCO2.level..µatm..ppm.=ifelse(experimental_pCO2.level..µatm..ppm. == 909.25 & Original.ref.number == 84, NA, experimental_pCO2.level..µatm..ppm.) ) %>% 
    mutate(experimental_pCO2.level..µatm..ppm.=ifelse(experimental_pCO2.level..µatm..ppm. == 909.25 & Original.ref.number == 159, NA, experimental_pCO2.level..µatm..ppm.) ) %>% 
    mutate(experimental_pCO2.level..µatm..ppm.=ifelse(experimental_pCO2.level..µatm..ppm. == 909.25 & Original.ref.number == 45, NA, experimental_pCO2.level..µatm..ppm.) ) 
  
  
mes<- dat1.5 %>% 
  #  filter(variable.label.fix.2.fix_PAM=="growth")
  #  filter(Original.ref.number==84)
    filter(complete.cases(Experimental_temp.level.degrees_fix))
...
mess1<-dat1 %>%   # names(dat1)
  ...
  dplyr::summarise(Factorial_experiment=n())  %>% 
...
temp_exp_k1<-(k1*(273+dat1$experimental_temp.level..degrees.))^-1
 ...
 ## some studies have true 0 in variations, add small number to no get "inf"
 dat1$experimental_variation_good_positive[dat1$experimental_variation_good_positive == 0]<- 0.0001
 dat1$control_variation_good_positive[dat1$contro_variation_good_positive == 0]<- 0.0001
...
#   switch variations
dat1 <- dat1 %>%     # names(dat1)    dat1$AE_9eV_temp_variation
  ...
  mutate(AE_9eV_temp_variation=ifelse( is.nan(AE_9eV_temp_variation) , NA, AE_9eV_temp_variation)) %>% 
  mutate(AE_9eV_temp_variation=ifelse( is.infinite(AE_9eV_temp_variation) , NA, AE_9eV_temp_variation))
...
vel1<-vel[,c(1,7,9,11,13,15,17)]   # names(vel)
vel1<-dplyr::rename(vel1,vel_average_sst=average_sst.1,vel_linear_change=linear_change.1,vel_spatial_gradient=spatial_gradient.1,
                    vel_angle=angle.1,vel_velocity_magnitude=velocity_magnitude.1, sstanom=sstanom.1)
##    names(dat3) names(land)   names(vel1)    names(abiotic)
dat4 <- dat3 %>% 
  left_join(abiotic[,c(1,7:length(names(abiotic)))], by="unique_study_loc") %>% 
  left_join(land[,c(1,7:13,15,16)], by="unique_study_loc") %>% # removed 13 - old sstanom from OHI
  left_join(vel1, by="unique_study_loc") 
...
dat5 <- dat4 %>% 
  dplyr::select(one_of(names_final_dat$Columns.required)) %>% 
  rename_at(vars(names_final_dat$Columns.required), ~names_final_dat$Colums.required.fix) %>% 
  ## fix some names
  mutate(Trait.type=if_else(Trait.type=="bleaching", "symbiont density", Trait.type)) %>% 
  mutate(Trait.type=if_else(Trait.type=="pam", "photochemical efficiency", Trait.type)) %>% 
  mutate(Trait.type=if_else(Trait.type=="PAM", "photochemical efficiency", Trait.type))

unique(dat5$Trait.type)
  
##   add back in columns for environmental match manuscript
mes<- dat4 %>% 
  dplyr::select(Present.Surface.Temperature.Lt.max,Present.Surface.Temperature.Range,sstanom,
                                       msec_humanpop_50km,yrange,
                                       Present.Surface.Salinity.Mean, coral_region_ID,  
                                    coral_region, coral_region_coral_richness,
                                       vel_linear_change, ECO_CODE, ECOREGION, PROV_CODE, PROVINCE)


dat5.5 <- dat5 %>% # names(dat5.5)
  bind_cols(mes)
...
write.table(ref1, paste(save_file,"references_fixed.csv", sep=""), sep=",",row.names=F) # names(ref1) hist(ref1$Year)
...
write.table(dat5.5, paste(save_file,"data_for_analysis_withabiotic_June2020.csv", sep=""), sep=",",row.names=F)   ## names(dat4)
