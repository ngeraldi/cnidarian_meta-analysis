...
>>>>>>> d61dbefe4d560152b660b413f9106007f485453d
```
...
<<<<<<< HEAD
 ...
 #  mutate(CI_estimate=((exp(ci.lb*relative_prop))-(exp(ci.ub*relative_prop)))/2) %>% # get project 95% CI previous old calc-bad
   ...
   # mutate(CI_estimate= mean_prop_estimate + first_ci ) %>%  # orignial diff + new mean_AE
...
>>>>>>> d61dbefe4d560152b660b413f9106007f485453d
  ...
  mutate(Sigificant=if_else( mean_prop_estimate+ abs(CI_estimate) < 1,"yes","no"))  %>%   # does 95% CI overlap with 0
...
for (j in c(1:length(op2$Estimate)) ) {
  ...
  for (i in seq(1,length(s),by=1)) {
   #  i<-43  j<-30
  ii<-i/100  # determein degree incremnt step - 
 ...
 # first_ci_old<-(  exp(op2$ci.lb[j] * -0.128710787*0.01  -0.003138539)  ) -  (    exp(op2$Estimate[j] * -0.128710787*  0.01  -0.003138539  )  )
  ...
  dum$prop_CI_AE[i]<- dum$prop_mean_AE[i] + first_ci   # orignial diff + new mean_AE
  ...
  if (op2$response[j]=="temperature"){
    ...
    less1<-dum2[dum2$prop_CI_AE <= 1  , ] # set cutoff- always 1
...
#### export meta-analysis summary table to shared folder
    write.table(op, paste(save_file_shared,"meta-analysis_summary_mar_2020_github.csv",sep=""), sep=",",row.names=F)

#### export rcp projection table to shared folder
    write.table(rcp_proj, paste(save_file_shared,"mhw_rcp_projections_from_meta.csv",sep=""), sep=",",row.names=F)
    
    #### export meta-analysis summary table to shared folder
    op2.2 <- op2 %>% 
      select(-finding)
    
    write.table(op2.2, paste(save_file_shared,"meta-analysis_summary_mar2020_with_future.csv",sep=""), sep=",",row.names=F)
```
...
hh<-c(0,0,0,0,h$counts,0,0,0)  #  c(0,0,h$counts,0)
