...
rcp_proj <-  data.frame(op,i=rep(1:length(rownames(rcp)),ea=NROW(op)))  %>%  # should be number of rcps?  op - out put of model
 ...
 #  mutate(CI_estimate=((exp(ci.lb*relative_prop))-(exp(ci.ub*relative_prop)))/2) %>% # get project 95% CI previous old calc-bad
   ...
   # respiration and growth have opposite repsonse to change - no diffreence
  ...
  # respiration and growth have opposite repsonse to change
...
#### export rcp projection table to shared folder
    write.table(rcp_proj, paste(save_file_shared,"mhw_rcp_projections_from_meta_may2020.csv",sep=""), sep=",",row.names=F)
    
    #### export meta-analysis summary table to shared folder
    op2.2 <- op2 %>% 
      select(-finding)
    
    write.table(op2.2, paste(save_file_shared,"meta-analysis_summary_may2020_with_future.csv",sep=""), sep=",",row.names=F)
    
    
# export data used in analysis   - use   sdat
    write.table(sdat, paste(save_file_shared,"data_used_in_meta-analysis__may2020.csv",sep=""), sep=",",row.names=F)
